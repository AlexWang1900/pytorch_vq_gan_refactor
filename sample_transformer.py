...
args.dataset_path = './data/FFHQ_128'
args.checkpoint_path = "./checkpoints/vqgan_epoch_99.pt"

n = 100
transformer = VQGANTransformer(args).to("cuda")
transformer.load_state_dict(torch.load(os.path.join("checkpoints", "transformer_99.pt")))
print("Loaded state dict of Transformer")

for i in tqdm(range(n)):
    start_indices = torch.zeros((4, 0)).long().to("cuda")#([1, 0])
    sos_tokens = torch.ones(start_indices.shape[0], 1) * 0#[1,1]
    sos_tokens = sos_tokens.long().to("cuda")
    sample_indices = transformer.sample(start_indices, sos_tokens, steps=64)#256
    sampled_imgs = transformer.z_to_image(sample_indices)
    vutils.save_image(sampled_imgs.mul(0.5).add(0.5), os.path.join("results", "transformer", f"transformer_{i}.png"), nrow=4)#,format="png"
